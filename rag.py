...
# Creates an OpenAI-like client but points to Ollama running locally (not OpenAI cloud). Use model llama3(Actually 3.2 copied to 3 locally)
client = OpenAI(base_url="http://localhost:11434/v1", api_key="llama3")

# Setup local ChromaDB client- vector DB; creates a collection - valut; Doc to vector embedding using all-MiniLM-L6-v2
...
# SPlits text into sentences to create manageable chunks for embeddings; change max_len and overlap as required based on the result and performance
...
# Read the pdf and chunk and stores in chromadb
...
# Read the txt and chunk and stores in chromadb
def process_txt(file):
    text = file.read().decode("utf-8")
    chunks = normalize_and_chunk(text)
    return process_chunks(chunks)

# Queries chromadb with the user's question and retrieves top_k most relavant chunks
def get_relevant_context(query, top_k=5):
    results = collection.query(query_texts=[query], n_results=top_k)
    return results['documents'][0] if results['documents'] else []

# Prepares a conversation with system prompt , user query and context; Send to LLM and get response back
...
# File Upload Handler; Drag and drop the files in the streamlit UI and see the chunks added to the chromadb
...
# Chat Interface; Remove the context print line as needed
